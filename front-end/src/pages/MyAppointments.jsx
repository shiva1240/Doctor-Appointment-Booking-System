...
const MyAppointments = () => {
  ...
  const getUserAppointments = async ()=>{
    try {
      ...
      console.log("Error in loading appointments",error);
  ...
  const appointmentRazorpay = async(appointmetId)=>{
    try {
      const {data} = await axios.post(backendUrl +'/api/user/payment-razorpay',{appointmetId},{headers:{token}})
      if(data.success)
      {
        initPay(data.order)
      }
          ...
          appointments.slice(0,3).map((item,index)=> (
            <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
                ...
                <div className='flex flex-col gap-2 justify-end'>
                 ...
                 {!item.cancelled && !item.payment  && !item.isCompleted  &&  <button onClick={()=>appointmentRazorpay(item._id)} className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-green-700 hover:text-white transition-all duration-300 '>Pay Online</button>} 
